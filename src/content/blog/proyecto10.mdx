...
<h1 slot="title">
  ...
  <span class="text-brand-500 [-webkit-text-fill-color:var(--color-brand-500)]">
    <TypingEffect
      words={['Web Designer', 'Web Developer', 'Astro Developer', 'Blogger', 'Coffee lover']}
    />
  </span>
</h1>
```
...
The fix wraps the entire animation in a `startTyping()` function and registers it on `astro:page-load`, which Astro fires on both the initial load _and_ every subsequent client-side navigation:
...
| ------------------ | ------------ | ------------------------------------------- |
| `words`            | _(required)_ | Array of strings to cycle through           |
...
<span
  id="typing-abc123"
  class="typing-effect"
  aria-label="Web Designer, Web Developer, Astro Developer, Blogger, Coffee lover"
></span>
```
